...
4.  It calls the **GitHub API** to fetch the latest repository information and any new commits since the last check.
5.  Finally, it saves this new data into the **PostgreSQL Database (`db`)**, where it can be easily queried.
...
.
├── cmd/service/        # Main application entry point. Wires everything together.
├── internal/           # All private application code lives here.
│   ├── config/         # Logic for loading configuration from the .env file.
│   ├── database/       # Database interaction layer (generated by sqlc).
│   ├── github/         # A dedicated client for interacting with the GitHub API.
│   ├── model/          # Core Go structs (Repository, Commit) for our application.
│   └── syncer/         # The core "brain" of the service; orchestrates fetching and storing.
├── migrations/         # SQL files for creating and updating the database schema.
├── .env.example        # An example configuration file.
├── Dockerfile          # Instructions for building the Go application into a container image.
└── docker-compose.yml  # Defines the services (app, db) and how they run together.